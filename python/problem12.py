...
def ways_to_climb_dp(n, steps, mem):
    if n < 0:
        return 0
    elif mem[n] != -1:
        return mem[n]
        ...
        value = 0
        for s in steps:
            value += ways_to_climb_dp(n-s, steps, mem)
    mem[n] = value
    return value

def ways_to_climb(n, steps):
    mem = [ -1 for x in range(n+1) ]
    return ways_to_climb_dp(n, steps, mem)

print(ways_to_climb(4, [1, 2]))